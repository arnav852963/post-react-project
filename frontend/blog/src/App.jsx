        ...
        ;(async ()=>{
            try {
                ...
                if(!res  || !res?.data || !res?.data?.data || res?.data?.statusCode !==200 ) {
                    ...
                    if (isUserAuthenticated) setError((prev)=> {return {...prev , error:true ,  message: data.data.message + `error code ${data.statusCode}`}})
                    dispatch(logout())
                    navigate('/login')
                    return
                ...
                setLoading(false)
                if(isUserAuthenticated)setError((prev)=> {return { error:true ,  message: e.message}})
                dispatch(logout())
